...
// Use MutationObserver to hide buttons after the viewer is fully loaded
document.addEventListener("DOMContentLoaded", () => {
  const observer = new MutationObserver((mutations, obs) => {
    const buttonsToRemove = ['#editorFreeText', '#editorHighlight', '#editorInk']; // Replace with actual selectors
    buttonsToRemove.forEach(selector => {
      const button = document.querySelector(selector);
      if (button) {
        const hiddenDummy = document.createElement('div');
        hiddenDummy.style.display = 'none';
        button.parentNode.replaceChild(hiddenDummy, button);
      }
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Hide and disable the print button
